...
public class MongoDBToMySQLtest2 {
...
public static void main (String[] args) throws Exception{
		...
		//BasicDBObject query = new BasicDBObject("vm_name", selectedvm);
		
		BasicDBObject query2 =new BasicDBObject("time", new BasicDBObject ("$gte", selectedtime));
		
		DBCursor cursor = coll.find(query2);

		// calculate average cpu usage in the past 1 mins
		double sumcpu=0;
		int summemory=0;
		int count=0;
